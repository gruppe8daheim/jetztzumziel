# 2025-01-07
# Achtung: nur für Fernverkehr
# Prüft auf Fahrtausfälle heute auf ausgewählter Strecke.
...
def main():
    ...
    depStopName = getStopName(departureStationMainID)
    ...
    arrStopName = getStopName(arrivalStationMainID)
 ...
 #   time = "20:00:00"    # debug
    for trip in sorted(tripsFromDepToArrStation, key=lambda trip: trip["arrival_time"]):    # SCHNELLSTER ZUG zum Ziel (ab jetzt) wird zuerst angezeigt
      ...
      #  print(runningToday, trip["arrival_time"], route, trip["trip_id"])    # debug
      #  abfahrten = 0
      ...
      #      if runningToday == "yes":
       #         if time > depTime:
        ...
        if runningToday == "noAsException":
            print(route + " um " + depTime + " von " + depStopName + " nach " + arrStopName + " fährt heute (" + strftime("%d.%m.%Y", localtime()) + ") ausnahmsweise nicht.")
        if runningToday == "yesAsException":
            print(route + " um " + depTime + " von " + depStopName + " nach " + arrStopName + " fährt heute (" + strftime("%d.%m.%Y", localtime()) + ") ausnahmsweise.")
